...
function tomarCienciaLote(urlPag){
    if(!$(".checkAviso").is(":checked")){
        ...
        $("#btnmodalPrincipal").attr("onclick", "abrirModalTipoCiencia('"+urlPag+"')");
        $("#btnmodalSegundario").attr("onclick", "fecharModal('dialogoConfirmacaoPadrao')");
...
function alterarSenhaSigad(idAvisoPje,idExpediente,idProcesso,idAtuacao,urlPag)
{
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(objRetorno.retorno == 0){
                    ...
                    $("#btnModalAviso").attr("onclick", "alterarSenhaSigad("+idAvisoPje+", "+idExpediente+", "+idProcesso+", "+idAtuacao+",'"+urlPag+"')");
                    aviso(objRetorno.msg, 0);
                }
            }
        });
...
function consultarTeorComunicacaoEmLote(subst, urlPag){
    if($("#senhaPje").val()!=""){
        $('#dialogoFormularioPadrao').modal('hide');
        //var form = $("#formAutentPje");
        var form = $("#formAcao");
        ...
        for(var i = 0; i<checados.length; i++){
            $.ajax({
                ...
                type: "POST",
                datatype: 'json',
                url: '/pje_aviso_pendentes/movimentacao/'+checados[i].split(",",4)[0]+'/'+checados[i].split(",",4)[3]+'/0?trs=1',
                data: form.serialize(),
                success: function (response) {
                    var objRetorno = JSON.parse(response);
                    if((objRetorno.retorno == 0)||(objRetorno.retorno == 4)||(objRetorno.retorno == 5)){
                        ...
                        $.ajax({
                            ...
                            type: "POST",
                            datatype: 'html',
                            url: '/pje_aviso_pendentes/movimentacao/'+checados[i].split(",",4)[0]+'/'+checados[i].split(",",4)[3]+'/1?trs=1',
